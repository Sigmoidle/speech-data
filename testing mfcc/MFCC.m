...
numFrames = floor((totalSamples/samplesPerFrame)*2);
...
function processFrames(samplesPerFrame, hopSize, numFrames, data, bands, sampleRate)
    for frameNumber = 500 %should be 1:numFrames
        dataSlice = data(frameNumber*hopSize:frameNumber*hopSize+samplesPerFrame);
        makeMFCC(dataSlice, samplesPerFrame, bands, sampleRate);
    end
end


function makeMFCC(data, samples, bands, sampleRate)
    ...
    applyMelFilter(data, filterBands, bands)
end
